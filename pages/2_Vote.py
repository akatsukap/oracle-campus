...
# データ読み込み - uses caching internally
...
onchain_markets = [utils.normalize_onchain_market(m) for m in onchain_raw]

# Use shared function for merging markets
markets = utils.merge_markets(local_markets, onchain_markets)
...
# find selected market using shared utility function
mid = str(selected_market)
market = utils.get_market_by_id(markets, mid)
if not market:
...
if st.button("投票する（送信）"):
	...
	if market.get("source") == "onchain" and web3_mgr:
		...
		if amount > pts:
			...
			# increase aggregate using the shared market lookup
			local_market = utils.get_market_by_id(local_markets, market.get("id"))
			if local_market:
				if is_yes:
					local_market["yes_bets"] = int(local_market.get("yes_bets", 0)) + int(amount)
				else:
					local_market["no_bets"] = int(local_market.get("no_bets", 0)) + int(amount)
